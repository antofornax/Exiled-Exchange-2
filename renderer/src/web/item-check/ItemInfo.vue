<template>
  ...
  >
    ...
    <div
      v-if="weaponDPS"
      class="grid mx-auto gap-x-4 my-2"
      style="grid-template-columns: auto auto"
    >
      <div>{{ t("item.physical_dps") }}</div>
      <div class="text-right">{{ weaponDPS.phys }}</div>
      <div>{{ t("item.elemental_dps") }}</div>
      <div class="text-right">{{ weaponDPS.elem }}</div>
      <div>{{ t("item.total_dps") }}</div>
      <div class="text-right">{{ weaponDPS.total }}</div>
    </div>
  </div>
</template>
...
function stashSearch() {
  actions.findSimilarItems(props.item);
}
function openWiki() {
  actions.openWiki(props.item);
}
function openPoedb() {
  actions.openPoedb(props.item);
}
function openCoE() {
